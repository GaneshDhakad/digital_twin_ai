...
def _resolve_subject(db: Session, subject_name: str) -> Optional[Subject]:
    if not subject_name:
        return None
    ...
    if subj:
        return subj
    try:
        new_subj = Subject(name=subject_name, code=subject_name[:10].upper(), is_active=True)
        db.add(new_subj)
        db.flush()
        return new_subj
    except Exception:
        db.rollback()
        return db.query(Subject).first()


def add_study_activity(db: Session, user_id: UUID, data: StudyActivityCreate) -> StudyActivity:
    subj_obj = _resolve_subject(db, data.subject)
    activity = StudyActivity(
        user_id=user_id,
        subject_id=subj_obj.subject_id if subj_obj else None,
...
def get_study_activities(
    ...
    if subject:
        query = query.outerjoin(StudyActivity.subject_rel).filter(
            or_(
...
def get_study_activity_by_id(db: Session, activity_id: Any, user_id: UUID) -> Optional[StudyActivity]:
    parsed_id = activity_id
    if isinstance(activity_id, str):
        try:
            parsed_id = UUID(activity_id)
        except Exception:
            try:
                parsed_id = int(activity_id)
            except Exception:
                pass
    return db.query(StudyActivity).filter(
        StudyActivity.activity_id == parsed_id,
...
def update_study_activity(
    ...
    if data.subject is not None:
        subj_obj = _resolve_subject(db, data.subject)
        if subj_obj:
            activity.subject_id = subj_obj.subject_id
            activity.subject_rel = subj_obj
...
def delete_study_activity(db: Session, activity_id: Any, user_id: UUID) -> bool:
    activity = get_study_activity_by_id(db, activity_id, user_id)
    if not activity:
        return False
    db.delete(activity)
...
def get_study_summary(db: Session, user_id: UUID) -> StudySummary:
